...
def list_text(title: str, items: tuple[Item, ...]) -> str:
    ...
    lines.extend(f"{index}. {item.title}" for index, item in enumerate(items, 1))
    return "\n".join(lines)
...
def build_dispatcher(storage: Storage, settings: Settings) -> Dispatcher:
    ...
    async def submit(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        await callback.answer()
        if callback.message:
            await callback.message.edit_text("✍️ <b>Submit Promotion</b>\n\nSend the promotion text you want to submit.\n\nMaximum length: 1,000 characters. Keep it clear and useful. Do not send passwords, payment credentials or other private information.", reply_markup=submit_menu(), parse_mode="HTML")
    ...
    async def receive_submission(message: Message, state: FSMContext) -> None:
        ...
        try:
            ...
            return
        await state.clear()
        await message.answer(f"✅ <b>Submission received</b>\n\nReference: <code>SP-{submission_id:06d}</code>\nYour promotion has been saved for consideration.", reply_markup=result_menu("submit"), parse_mode="HTML")
...
async def main() -> None:
    ...
    try:
        await bot.set_my_commands([BotCommand(command="start", description="Open the Starz Promosyon menu"), BotCommand(command="help", description="Learn how the three functions work")])
